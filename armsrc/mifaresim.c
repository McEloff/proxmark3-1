    ...
    *rats = NULL;
    ...
    if ((flags & FLAG_UID_IN_EMUL) == FLAG_UID_IN_EMUL) {
        ...
        if ((flags & (FLAG_4B_UID_IN_DATA | FLAG_7B_UID_IN_DATA | FLAG_10B_UID_IN_DATA)) != 0) {
            ...
            // Check for 7 bytes UID: double size uid bits in ATQA
                ...
                if (DBGLEVEL >= DBG_ERROR)    Dbprintf("ERROR: " _RED_("Invalid dump. UID/SAK/ATQA not found"));
                return false;
            }
        }
    ...
    if ((flags & FLAG_4B_UID_IN_DATA) == FLAG_4B_UID_IN_DATA) { // get UID from datain
        ...
        *cuid = bytes_to_num(rUIDBCC3, 4);
        ...
        if (DBGLEVEL >= DBG_ERROR)    Dbprintf("ERROR: " _RED_("UID size not defined"));
        return false;
    }
...
*/
void Mifare1ksim(uint16_t flags, uint8_t exitAfterNReads, uint8_t exitAfterNWrites, uint8_t *datain, uint16_t atqa, uint8_t sak) {
    bool reinit = false;
...
#define COLLECTED_NONCES_COUNT 32 // max array size for save completed nonces for NR/AR attack
                ...
                if (receivedCmd_len == MAX_MIFARE_FRAME_SIZE) {
                    ...
                    if (CheckCrc14A(receivedCmd_dec, receivedCmd_len)) {
                        ...
                        if (exitAfterNWrites > 0 && numWrites >= exitAfterNWrites) {
                            ...
                            EmSend4bit(mf_crypto1_encrypt4bit(pcs, CARD_NACK_NA));
                            if (DBGLEVEL >= DBG_ERROR)    Dbprintf("%d writes done, last block %d (%02x), exiting", numWrites, cardWRBL, cardWRBL);
                            finished = true;
                            cardSTATE_TO_IDLE();
                        } else {
                            EmSend4bit(mf_crypto1_encrypt4bit(pcs, CARD_ACK));
    ...
    for (uint8_t i = 0; i < nonces_count; i++) {
        ...
        if ((flags & FLAG_INTERACTIVE) == FLAG_INTERACTIVE)
            reply_ng(CMD_HF_MIFARE_SIMULATE, CMD_HF_MIFARE_SIMULATE, (uint8_t *) &ar_nr_collected[i], sizeof(nonces_t));
        else {
    ...
    if ((flags & FLAG_INTERACTIVE) == FLAG_INTERACTIVE) {  // Interactive mode flag, means we need to send ACK
        ...
        reply_ng(CMD_HF_MIFARE_SIMULATE, PM3_SUCCESS, NULL, 0);
    }
