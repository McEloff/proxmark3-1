...
void MifareGen3UID(uint8_t uidlen, uint8_t *uid) {
    ...
    ReaderTransmit(cmd, sizeof(uid_cmd) + uidlen + 2, NULL);
...
#define MIFARE_BLOCK_SIZE (MAX_MIFARE_FRAME_SIZE - 2)
    ...
    if (block_len > 0) {
        ...
        ReaderTransmit(cmd, sizeof(block_cmd) + MAX_MIFARE_FRAME_SIZE, NULL);
        int res = ReaderReceive(buf, par);
        iso14a_set_timeout(save_iso14a_timeout); 
...
void MifareGen3Freez(void) {
    ...
    ReaderTransmit(freeze_cmd, sizeof(freeze_cmd), NULL);
    int res = ReaderReceive(buf, par);
    iso14a_set_timeout(save_iso14a_timeout); 
