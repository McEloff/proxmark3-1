...
static int CmdHF14AMfSim(const char *Cmd) {
    ...
    void *argtable[] = {
        ...
        arg_lit0("e", "emukeys", "Fill simulator keys from found keys"),
        arg_lit0("v", "verbose", "verbose output"),
        arg_lit0("q", "quiet", "Quiet, no trace log saves"),
        arg_int0("w", "writes", "<dec> ", "Automatically exit simulation after <numreads> blocks have been written by reader. 0 = infinite"),
        arg_param_end
    };
    CLIExecWithReturn(ctx, Cmd, argtable, true);
    ...
    uint8_t exitAfterNWrites = arg_get_u32_def(ctx, 14, 0);
    ...
    payload.flags = flags;
    payload.exitAfterReads = exitAfterNReads;
    payload.exitAfterWrites = exitAfterNWrites;
    ...
    if (flags & FLAG_INTERACTIVE) {
        ...
        while (!kbd_enter_pressed()) {
            if (!WaitForResponseTimeout(CMD_HF_MIFARE_SIMULATE, &resp, 1500)) continue;
            if (resp.status != CMD_HF_MIFARE_SIMULATE) break;
